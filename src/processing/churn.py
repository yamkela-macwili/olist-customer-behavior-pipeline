...
def compute_churn_label(
    ...
    obs_customers = set(df.loc[df["order_purchase_timestamp"] <= cutoff, "customer_unique_id"])
    outcome_customers = set(df.loc[df["order_purchase_timestamp"] > cutoff, "customer_unique_id"])

    labels = pd.DataFrame({
        "customer_unique_id": sorted(obs_customers),
    })
    labels["churned"] = labels["customer_unique_id"].apply(
        lambda cid: 0 if cid in outcome_customers else 1
    )

    labels.to_sql("customer_churn_label", con=engine, schema=target_schema, if_exists="replace", index=False)
    return len(labels)