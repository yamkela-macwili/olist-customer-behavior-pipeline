...
joined against Olist's actual geolocation table. 
"""
...
def build_order_geography(engine: Engine, source_schema: str = None, target_schema: str = None) -> int:
    ...
    df.to_sql("order_geography", con=engine, schema=target_schema, if_exists="replace", index=False)
    return len(df)