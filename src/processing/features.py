...
def compute_time_split_cutoff(engine: Engine, source_schema: str = None, quantile: float = 0.8) -> pd.Timestamp:
...
def _load_orders_with_customers(engine: Engine, source_schema: str = None) -> pd.DataFrame:
    orders = _qualify("orders", source_schema)
...
def compute_rfm(engine: Engine, cutoff: pd.Timestamp, source_schema: str = None, target_schema: str = None) -> int:
    ...
    """
    fact = _qualify("fact_orders", target_schema)  # fact_orders lives in the marts/target schema
...
def compute_delivery_experience(
    engine: Engine, cutoff: pd.Timestamp, source_schema: str = None, target_schema: str = None
) -> int:
    ...
    if len(df) == 0:
        empty = pd.DataFrame(columns=["customer_unique_id", "avg_delay_days", "late_delivery_rate"])
        empty.to_sql("customer_delivery_experience", con=engine, schema=target_schema, if_exists="replace", index=False)
        return 0
    ...
    result.to_sql("customer_delivery_experience", con=engine, schema=target_schema, if_exists="replace", index=False)
    return len(result)
